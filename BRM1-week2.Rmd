...
emancipation_items <- c("Q28", "Q29", "Q30", "Q31", "Q32", "Q33", "Q35")
emancipation_items_R <- c("Q28R", "Q29R", "Q30R", "Q31R", "Q32R", "Q33R", "Q35R")
...
question4b <- function() {
  ...
  m_country1 <- var(df[df$Country== country1, itemsR], use = "pairwise.complete.obs") 
  
  sum(diag(m_country1))
  
  # ANSWER: Print the variance-covariance matrix country 2
  m_country2 <- var(df[df$Country== country2, itemsR], use = "pairwise.complete.obs") 
 ...
 sum(diag(m_country2))
  ...
  print(((k/(k-1))*(1-sum(diag(m_country1))/sum(m_country1))))

  # ANSWER: Print the cronbach's alpha for country 2
  print(((k/(k-1))*(1-sum(diag(m_country2))/sum(m_country2))))
...
question4c <- function() {
  # get the correlationmatrix for each country across the seven recoded items
    ...
    # save as an R object
  corC<<-cor(df[df$Country == country1, c("Q28R", "Q29R", "Q30R","Q31R","Q32R","Q33R","Q35R")], use = "pairwise.complete.obs")
corP<<- cor(df[df$Country == country2, c("Q28R", "Q29R", "Q30R","Q31R","Q32R","Q33R","Q35R")], use = "pairwise.complete.obs")
...
meancorC <<- mean(corC[lower.tri(corP,diag = F)])
print((7*meancorC) / (1+(7-1) * meancorC))

meancorP<<- mean(corP[lower.tri(corC,diag = F)])
print((7*meancorP) / (1+(7-1) * meancorP))
...
emanci_DEU <- apply(df[df$Country== country1, c("Q28R", "Q29R", "Q30R", "Q31R", "Q32R", "Q33R", "Q35R")], 2, mean, na.rm =TRUE)

emanci_GTM <- apply(df[df$Country== country2, c("Q28R", "Q29R", "Q30R", "Q31R", "Q32R", "Q33R", "Q35R")], 2, mean, na.rm =TRUE)
  ...
  print(mean(abs(emanci_DEU)))
  # ANSWER: print mean new variable for country 2
   
  mean(abs(emanci_GTM))
  
  mean(emanci_DEU-emanci_GTM)
}
question5()
...
question6a()
  attach(df)
   ...
   emancipation_1 <- apply(df[df$Country==country1, c("Q28", "Q29", "Q30", "Q31", "Q32", "Q33", "Q35")], 1, mean, na.rm =TRUE)

print(plot(Q262[Country==country1], emancipation_1, main = "Relationship of age and emancipation Germany", xlab = "Age Germany", ylab="Emancipation Germany"))
...
question6b <- function() {
  ...
  emancipation_2 <- apply(df[df$Country==country2, c("Q28", "Q29", "Q30", "Q31", "Q32", "Q33", "Q35")], 1, mean, na.rm =TRUE)

print(plot(Q262[Country==country2], emancipation_2, main = "Relationship of age and emancipation Guatamala", xlab = "Age Guatamala", ylab="Emancipation Guatamala"))
...
question7 <- function() {
  ...
  "1" <- ages[which(df$Q262 >=15) & (df$Q262 <= 34)]
